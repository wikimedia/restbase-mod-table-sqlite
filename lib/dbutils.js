    ...
    for (var predKey in pred) {
        ...
        if (predObj === undefined) {
            ...
            if (predKeys.length === 1) {
                ...
                case 'eq': cql += ' = ?'; params.push(convert(predKey, predObj)); keys.push(predKey); break;
                case 'lt': cql += ' < ?'; params.push(convert(predKey, predObj)); keys.push(predKey); break;
                case 'gt': cql += ' > ?'; params.push(convert(predKey, predObj)); keys.push(predKey); break;
                case 'le': cql += ' <= ?'; params.push(convert(predKey, predObj)); keys.push(predKey); break;
                case 'ge': cql += ' >= ?'; params.push(convert(predKey, predObj)); keys.push(predKey); break;
                case 'neq':
                case 'ne': cql += ' != ?'; params.push(convert(predKey, predObj)); keys.push(predKey); break;
                case 'between':
...
dbu.buildPutQuery = function(req, keyspace, table, schema, action) {
    ...
    if (action === 'insert' && !req.if || req.if === 'not exists') {
        ...
        if (staticKeys.length) {
            condRes = dbu.buildCondition(sindexKVMap. schema);
