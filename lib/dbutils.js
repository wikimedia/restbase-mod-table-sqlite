...
dbu.buildPutQuery = function(req, action) {
    ...
    if (Object.keys(schema.secondaryIndexes).length>0) {
        ...
        var cql3 = 'update ' + dbu.cassID(req.columnfamily);
    ...
    if (action === 'insert' && !query.if || query.if === 'not exists') {
        ...
        cql = 'insert ' + cond + ' into ' + dbu.cassID(req.columnfamily)
                + ' (' + proj + ') values (';
        ...
        if (staticKeys.length) {
            ...
            cql2 = 'insert into ' + dbu.cassID('static')
                + ' (' + proj + ') values (';
        ...
        cql += 'update ' + dbu.cassID(req.columnfamily)
...
dbu.buildGetQuery = function(req) {
    ...
    if (proj) {
        ...
        if (Array.isArray(proj)) {
            ...
            proj = proj.map(function(elem){return dbu.cassID(req.columnfamily)+'.'+elem}).join(',');
    ...
    if (query.attributes) {
        if (staticsCond) {
            condResult = dbu.buildCondition(query.attributes, schema, req.columnfamily);
            cql += ', ' + dbu.cassID('static');
            var newReq = []
            Object.keys(hashKeys).forEach(function(elem){
                newReq.push(dbu.cassID(req.columnfamily)+'.'+elem+' = '+
