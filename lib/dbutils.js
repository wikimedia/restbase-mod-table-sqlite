...
dbu.buildCondition = function buildCondition (predicates, schema, table) {
    ...
    for (var predKey in predicates) {
        ...
        if (table) {
            sql += dbu.cassID(table) +"."+ dbu.cassID(predKey);
        } else {
            sql += dbu.cassID(predKey);
        }
        if (predObj === undefined) {
            ...
            if (predKeys.length === 1) {
                ...
                case 'between':
                        ...
                        sql += dbu.cassID(predKey) + ' <= ?'; params.push(convert(predKey, predArg[1])); keys.push(predKey);
                        break;
...
dbu.buildPutQuery = function(req, action) {
    ...
    if (Object.keys(schema.secondaryIndexes).length>0) {
        ...
        var sql3 = 'update ' + dbu.cassID(req.sqlTable);
        sql3 += ' set _is_latest = ? where ' + schema.tid + ' != ? ';
        sql3 += ' and ' + condRes.sql;
    ...
    if (action === 'insert' && !query.if || query.if === 'not exists') {
        ...
        if (staticKeys.length) {
            ...
            sql2 = 'insert into ' + dbu.cassID('static')
                + ' (' + proj + ') values (';
            sql2 += placeholders.join(',') + ')' + using;
            params2 = params2.concat(staticParams, usingParams);
            if (Object.keys(schema.secondaryIndexes).length>0) {
                return [{sql:sql, params:params}, {sql:sql2, params:params2}, {sql:sql3, params: params3}];
            }
            return [{sql:sql, params:params}, {sql:sql2, params:params2}];
        }
        ...
        sql += 'update ' + dbu.cassID(req.sqlTable)
        ...
        if (staticKeys.length) {
            ...
            sql2 = 'update ' + dbu.cassID('static')
                + using + ' set ' + updateProj + ' where ';
            sql2 += condRes.sql + cond;
    ...
    if (Object.keys(schema.secondaryIndexes).length>0) {
        return [{sql:sql3, params: params3}, {sql: sql, params: params}]; 
    }
...
dbu.buildGetQuery = function(req) {
        ...
        if (Array.isArray(proj)) {
            ...
            if (Object.keys(statics).length) {
                ...
                var staticsCond = statics.map(function(elem){
                            return dbu.cassID('static')+'.'+elem
                          }).join(',');
            }
            proj = proj.map(dbu.cassID);
            proj = proj.map(function(elem){return dbu.cassID(req.sqlTable)+'.'+elem}).join(',');
    ...
    if (query.attributes) {
        if (staticsCond) {
            ...
            sql += ', ' + dbu.cassID('static');
            var newReq = []
            Object.keys(hashKeys).forEach(function(elem){
                newReq.push(dbu.cassID(req.sqlTable)+'.'+elem+' = '+
                            dbu.cassID('static')+'.'+elem);
            });
            sql += ' ON ';
            sql += newReq.join(' AND ');
