...
dbu.buildPutQuery = function(req, action) {
    ...
    if (Object.keys(schema.secondaryIndexes).length>0) {
        ...
        params.push('true');
        var cql3 = 'update ' + dbu.cassID(req.keyspace + '_' + req.columnFamily);
        cql3 += ' set _is_latest = ? where ' + schema.tid + ' != ? ';
        cql3 += ' and ' + condRes.cql;
        var params3 = ['false', uuid.unparse(attributes[schema.tid])].concat(condRes.params);
        console.log(cql3, params3);
    }
    var using = '';
    var usingParams = [];
    var usingParamsKeys = [];
    //TODO- see http://stackoverflow.com/questions/14461851/how-to-have-an-automatic-timestamp-in-sqlite
    /*if (query.timestamp && !req.if) {
    ...
    if (action === 'insert' && !query.if || query.if === 'not exists') {
        ...
        cql = 'insert ' + cond + ' into ' + dbu.cassID(req.keyspace + '_' + req.columnfamily)
                + ' (' + proj + ') values (';
        ...
        if (staticKeys.length) {
            ...
            cql2 = 'insert into ' + dbu.cassID(req.keyspace + '_' + req.columnfamily  + '_static')
                + ' (' + proj + ') values (';
            cql2 += placeholders.join(',') + ')' + using;
            params2 = params2.concat(staticParams, usingParams);
            if (Object.keys(schema.secondaryIndexes).length>0) {
                return [{cql:cql, params:params}, {cql:cql2, params:params2}, {cql:cql3, params: params3}];
            }
            return [{cql:cql, params:params}, {cql:cql2, params:params2}];
        }
        ...
        cql += 'update ' + dbu.cassID(req.keyspace + '_' + req.columnfamily)
        ...
        if (staticKeys.length) {
            ...
            cql2 = 'update ' + dbu.cassID(req.keyspace + '_' + req.columnfamily  + '_static')
                + using + ' set ' + updateProj + ' where ';
            cql2 += condRes.cql + cond;
    ...
    if (Object.keys(schema.secondaryIndexes).length>0) {
        return [{cql:cql3, params: params3}, {cql: cql, params: params}]; 
    }
...
dbu.buildGetQuery = function(req) {
    ...
    if (query.proj) {
        if (query.proj === "*") {
            query.proj = Object.keys(schema.attributes);
        }
        if (Array.isArray(query.proj)) {
            ...
            if (Object.keys(statics).length) {
                ...
                var staticsCond = statics.map(function(elem){
                            return dbu.cassID(req.keyspace+'_'+req.columnfamily+'_static')+'.'+elem
                          }).join(',');
            }
            proj = proj.map(dbu.cassID);
            proj = proj.map(function(elem){return dbu.cassID(req.keyspace+'_'+req.columnfamily)+'.'+elem}).join(',');
    ...
    var cql = 'select ' + proj + ' from '
        + dbu.cassID(req.keyspace +'_'+ req.columnfamily);
    if (req.index) {
        cql += idxCond + ' ';
    }
    ...
    if (query.attributes) {
        if (staticsCond) {
            condResult = dbu.buildCondition(query.attributes, schema, req.keyspace+'_'+req.columnfamily);
            cql += ', ' + dbu.cassID(req.keyspace +'_'+ req.columnfamily + '_static');
            var newReq = []
            Object.keys(hashKeys).forEach(function(elem){
                newReq.push(dbu.cassID(req.keyspace+'_'+req.columnfamily)+'.'+elem+' = '+
                            dbu.cassID(req.keyspace+'_'+req.columnfamily+'_static')+'.'+elem);
            });
            cql += ' ON ';
            cql += newReq.join(' AND ');
            cql += ' AND '  + condResult.cql;
