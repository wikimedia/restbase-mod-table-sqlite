...
dbu.conversions = {
    ...
    boolean: {
        read: function(value) { return value !== 0;},
        write: function(value) { return value ? 1 : 0},
        type: 'integer'
    },
    decimal: {
        read: toString(),
...
function generateConverters(schema) {
    ...
    Object.keys(schema.attributes).forEach(function(key) {
        ...
        if (!obj_converter.write) {
            obj_converter.write = function(val) { return val; };
        }
        if (!obj_converter.read) {
            obj_converter.read = function(val) { return val; };
        }
...
function constructGroupingByHashKeys(query, schema) {
...
dbu.buildGetQuery = function(keyspace, query, table, schema) {
    ...
    if (query.index) {
        var grouping = constructGroupingByHashKeys(query, schema);
        sql = 'select ' + proj + ' from ( select * from ' + keyspace + '_' + table;
        if (isStaticJoinNeeded(query, schema, table)) {
            sql += ' natural left outer join ' + keyspace + '_static';
        }
        sql += grouping + order + ')' + condition + limit;
    } else {
        if (isStaticJoinNeeded(query, schema, table)) {
