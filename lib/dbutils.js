...
dbu.buildCondition = function buildCondition (pred, table) {
    var params = [];
    var keys = [];
    var conjunctions = [];
...
dbu.buildPutQuery = function(req, keyspace, table, schema, action) {
    ...
    if (action === 'insert' && !req.if || req.if === 'not exists') {
        ...
        if (staticKeys.length) {
            condRes = dbu.buildCondition(sindexKVMap);
...
dbu.buildGetQuery = function(keyspace, req, table, schema) {
    ...
    if (req.attributes) {
        if (staticsCond) {
            condResult = dbu.buildCondition(req.attributes, keyspace+'_'+table);
            ...
            condResult = dbu.buildCondition(req.attributes);
            cql += ' where ';
