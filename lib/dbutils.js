...
dbu.buildGetQuery = function(keyspace, query, table, schema) {
    ...
    if (query.proj) {
        ...
        && Object.keys(schema.iKeyMap).some(function(key) {
    ...
    Object.keys(schema.iKeyMap).forEach(function(key) {
        ...
        if (elem.type === 'range') {
            var dir = query.order && query.order[elem.attribute]
...
dbu.buildPutQuery = function(req, keyspace, table, schema) {
    ...
    if (req && req.attributes) {
        Object.keys(req.attributes).forEach(function(key) {
            req.attributes[key] = dbu.convertToSQLite(req.attributes[key], schema.attributes[key])
        });
    }
...
dbu.buildCondition = function(pred, schema) {
    var params = [];
    var conjunctions = [];
    Object.keys(pred).forEach(function(predKey) {
        ...
        if (predObj === undefined) {
            ...
            // Default to equality
            sql += ' = ?';
            params.push(dbu.convertToSQLite(predObj, schema.attributes[predKey]));
        } else {
            ...
            if (predKeys.length === 1) {
                ...
                switch (predOp.toLowerCase()) {
                    case 'eq':
                        sql += ' = ?';
                        params.push(dbu.convertToSQLite(predArg, schema.attributes[predKey]));
                        break;
                    case 'lt':
                        sql += ' < ?';
                        params.push(dbu.convertToSQLite(predArg, schema.attributes[predKey]));
                    ...
                    case 'between':
                        sql += ' >= ?' + ' AND ';
                        params.push(dbu.convertToSQLite(predArg[0], schema.attributes[predKey]));
                        sql += dbu.fieldName(predKey) + ' <= ?';
                        params.push(dbu.convertToSQLite(predArg[1], schema.attributes[predKey]));
                        break;
                    default:
...
dbu.buildStaticsTableSql = function(schema, keyspace) {
    ...
    sql += hashKeys.concat(staticFields).map(function(key) {
        return dbu.fieldName(key) + ' ' + dbu.getSQLiteType(schema.attributes[key])
    }).join(', ');
...
dbu.buildTableSql = function(schema, keyspace, table) {
    ...
    .map(function(attr) {
        ...
        return dbu.fieldName(attr) + ' ' + dbu.getSQLiteType(schema.attributes[attr]);
    })
    .join(', ');
...
dbu.getSQLiteType = function(type) {
    if (type.indexOf('set') === 0 || type === 'json') {
        type = 'blob';
...
dbu.convertToSQLite = function(value, type) {
    if (type.indexOf('set') === 0 || type === 'json') {
        if (value) {
            value = JSON.stringify(value) // TODO: handle that it's a set
        }
    } else if (type === 'boolean') {
        value = value ? 1 : 0;
        value = value !== 0;
...
dbu.convertFromSQLite = function(value, type) {
    if (type.indexOf('set') === 0 || type === 'json') {
        if (value) {
