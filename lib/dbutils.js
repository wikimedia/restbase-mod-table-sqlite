...
dbu.buildPutQuery = function(req, keyspace, table, schema, action) {
    ...
    Object.keys(schema.secondaryIndexes).forEach(function(index) {
        schema.secondaryIndexes[index].iKeys.forEach(function(key) {;
            if (schema.secondaryIndexes[index].iKeyMap[key].type==='hash' && key !== schema.tid) {
                sindexKVMap[key] = {'neq':req.attributes[key]};
            }
        });
    });
