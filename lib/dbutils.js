...
dbu.makeIndexSchema = function makeIndexSchema(dataSchema, indexName) {
    ...
    if (dataSchema.conversions) {
        Object.keys(s.attributes).forEach(attr => {
...
function findTidElement(schema) {
    for (const key in schema.index) {
        if (schema.index.hasOwnProperty(key)) {
            const element = schema.index[key];
...
dbu.makeSchemaInfo = function makeSchemaInfo(schema) {
    ...
    const tidAttribute = findTidElement(schema);
...
function constructOrder(query, schema) {
    ...
    Object.keys(schema.iKeyMap).forEach(key => {
        ...
        if (elem.type === 'range') {
            const dir = query && query.order && query.order[elem.attribute]
...
dbu.buildGetQuery = (tableName, query, schema, includePreparedForDelete) => {
    ...
    if (query.index) {
        const indexSchema = schema.secondaryIndexes[query.index];
        proj = constructProj(query, indexSchema);
        sql = `select ${proj} from [${tableName}_secondaryIndex]`
...
function buildUpdateQuery(req, tableName, schema, dataKVMap, primaryKeyKVMap, ignore) {
    ...
    sql += Object.keys(dataKVMap).filter(column => schema.iKeys.indexOf(column) < 0).map(column => {
...
dbu.buildSecondaryIndexUpdateQuery = (req, tableName, schema) => {
    const result = [];
    ...
    .filter(key => schema.allSecondaryIndexKeys.has(key))
    .forEach(key => {
        dataKVMap[key] = req.attributes[key];
    });
    sql += `${Object.keys(dataKVMap).map(dbu.fieldName).join(', ')}) values (`;
    sql += `${Object.keys(dataKVMap).map(() => '?').join(', ')})`;
    result.push({
        sql,
        params: Object.keys(dataKVMap).map(key => dataKVMap[key])
    });
...
dbu.buildSecondaryIndexTableSql = (schema, tableName) => {
    const result = [];
    const secondaryIndexNames = Object.keys(schema.secondaryIndexes);
    let tableSql;
    if (secondaryIndexNames.length === 0) {
    ...
    schema.allSecondaryIndexKeys.forEach(attr => {
    ...
    Object.keys(schema.secondaryIndexes).forEach(indexName => {
        const indexSchema = schema.secondaryIndexes[indexName];
        let indexSql = `create index if not exists`;
        indexSql += ` ${dbu.indexOverSecIndexName(tableName, indexName)}`;
...
dbu.buildDeleteQuery = (tableName, keys) => {
    let sql = `delete from [${tableName}_data] where `;
    const params  = [];
    sql += Object.keys(keys).map(key => {
