...
class SecondaryIndexes {
    constructor(parentMigrator, current, proposed) {
        ...
        new Set(Object.keys(current).concat(Object.keys(proposed))).forEach((indexName) => {
            if (!proposed[indexName]) {
                this.deletedIndexes.push(indexName);
            } else if (!current[indexName]) {
                this.addedIndexes.push(indexName);
            } else if (!this._isEqual(current[indexName], proposed[indexName])) {
                this.changedIndexes.push(indexName);
            }
        });
    }

    validate() {
        if (this.addedIndexes.length > 0) {
    ...
    migrate() {
        return this.deletedIndexes.forEach((indexName) => {
            this.log('warn/schemaMigration/secondaryIndexes', {
                message: `deleting secondary index ${indexName}`,
                index: indexName
            });
            const sql = this._removeIndexTable(indexName);
            return this.client.run([{ sql }]);
        });
    }

    _removeIndexTable(indexName) {
        ...
        return `drop index if exists ${dbu.indexOverSecIndexName(this.table, indexName)}`;
    }
}

SecondaryIndexes.prototype._isEqual = (currentIndex, proposedIndex) =>
    stringify(currentIndex) === stringify(proposedIndex);
