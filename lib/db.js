...
DB.prototype.get = function (reverseDomain, req) {
    ...
    if (!schema) {
        ...
        return this._get(keyspace, req, 'data', schema);
    }
};
...
DB.prototype._get = function (keyspace, req, table, schema) {
    ...
    return self.client.all_p(buildResult.query, buildResult.params)
    .then(function(result){
        var rows = [];
        if (result instanceof Array) {
            result.forEach(function(row) {
                // Apply value conversions
                row = dbu.convertRow(row, schema);
...
DB.prototype._put = function(keyspace, req, table, action ) {
    ...
    if (schema.secondaryIndexes) {
        for ( var idx in schema.secondaryIndexes) {
            ...
            batch = dbu.buildPutQuery(req, keyspace, dbu.idxTable(idx), secondarySchema, action);
        }
    }
    batch = dbu.buildPutQuery(req, keyspace, table, schema, action);
    ...
    if (batch.length === 1) {
        ...
        mainUpdate = new Promise(function(resolve,reject) {
            self.client.serialize(function() {
                self.client.run('begin transaction;');
                self.client.run(batch[0].query, batch[0].params, function(err){
                    if (err instanceof Object && err.code === 'SQLITE_CONSTRAINT') {
                        return self._put(keyspace, req, table, "update");
                    }
                });
                self.client.run(batch[1].query, batch[1].params, function(err){
                    if (err instanceof Object && err.code === 'SQLITE_CONSTRAINT') {
                        return self._put(keyspace, req, table, "update");
                    }
                });
...
DB.prototype._createTable = function (keyspace, schema, tableName) {
    ...
    if (Object.keys(statics).length) {
        ...
        + dbu.cassID(keyspace + '_' + tableName + '_static') + ' (';
    }
    ...
    if (sql2) {
        ...
        sql2 += ' FOREIGN KEY( '+ hashBits.join(',') +' ) REFERENCES '+ dbu.cassID(keyspace + '_' + tableName);
        sql2 += ' (' + hashBits.join(',') + ')) '

        sql = 'begin transaction; ' + sql1 + '; '+ sql2 +';  end transaction;'
    } else {
        sql = sql1;
    }
    // console.log(sql);
