...
DB.prototype.createTable = function (domain, query) {
    ...
    .then(function(req) {
        ...
        // The code below is a workaround implemented in restbase cassandra module.
        // The work-around is to retry a few times before giving up.  Our
        // table creation code is idempotent, which makes this a safe thing to do.
        // 
        // The below is significant on the first restbase startup, when many workers
...
DB.prototype._createTable = function (req, schema, tableName) {
    ...
    schema.index.forEach(function(elem) {
        ...
        if (elem.type === 'hash') {
            ...
            rangeBits.push(attr);
            orderBits.push(attr + ' ' + elem.order);
        }
    });
