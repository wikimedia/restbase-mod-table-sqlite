...
DB.prototype._makeInternalRequest = function (domain, table, query) {
    ...
    req.dbClient = this.dblists[req.keyspace];

    if (req.schema && req.dbClient) {
        return P.resolve(req);
    } else {
        if (!req.dbClient) {
            var client = new sqlite3.Database(req.keyspace);
            this.dblists[req.keyspace] = client;
            req.dbClient = client;
        }
        var schemaQuery = {
            attributes: {
...
DB.prototype._get = function (req) {
    ...
    //    });
    //}
    return req.dbClient.all_p(buildResult.cql, buildResult.params)
...
DB.prototype.createTable = function (domain, query) {
    ...
    .then(function(req) {
        ...
        // SQLite does not like concurrent keyspace creation. This is
...
DB.prototype._createTable = function (req, schema, tableName) {
    ...
    if (sql2) {
        ...
        sql2 += ' FOREIGN KEY( '+ hashBits.join(', ') +' ) REFERENCES '+ dbu.cassID(tableName);
        sql2 += ' (' + hashBits.join(',') + ')) ';
...
DB.prototype._dropTable = function (req) {
    var keyspace = dbu.keyspaceName(req.domain, req.table);
    if (!req.schema) {
    ...
    if (hasStatic) {
        return req.dbClient.exec_p('drop table ' + dbu.cassID("data") + 
                                      "; drop table" + dbu.cassID("meta") +
                                      "; drop table" + dbu.cassID(+"static"));
    }
    return req.dbClient.exec_p('drop table ' + dbu.cassID("data") + "; drop table" + dbu.cassID("meta"));
}
