...
DB.prototype._put = function(keyspace, req, table, action ) {
    ...
    if (schema.secondaryIndexes) {
        for ( var idx in schema.secondaryIndexes) {
            ...
            batch.push(dbu.buildPutQuery(req, keyspace, dbu.idxTable(idx), secondarySchema, action));
        }
    }
    batch.push(dbu.buildPutQuery(req, keyspace, table, schema, action));
...
DB.prototype._createTable = function (keyspace, schema, tableName) {
    ...
    for (var attr in schema.attributes) {
        ...
        sql += dbu.cassID(attr) + ' ';
        switch (type) {
        case 'blob': sql += 'blob'; break;
        case 'set<blob>': sql += 'set<blob>'; break;
        case 'decimal': sql += 'decimal'; break;
        case 'set<decimal>': sql += 'set<decimal>'; break;
        case 'double': sql += 'double'; break;
        case 'set<double>': sql += 'set<double>'; break;
        case 'float': sql += 'float'; break;
        case 'set<float>': sql += 'set<float>'; break;
        case 'boolean': sql += 'boolean'; break;
        case 'set<boolean>': sql += 'set<boolean>'; break;
        case 'int': sql += 'int'; break;
        case 'set<int>': sql += 'set<int>'; break;
        case 'varint': sql += 'varint'; break;
        case 'set<varint>': sql += 'set<varint>'; break;
        case 'string': sql += 'text'; break;
        case 'set<string>': sql += 'set<text>'; break;
        case 'timeuuid': sql += 'timeuuid'; break;
        case 'set<timeuuid>': sql += 'set<timeuuid>'; break;
        case 'uuid': sql += 'uuid'; break;
        case 'set<uuid>': sql += 'set<uuid>'; break;
        case 'timestamp': sql += 'timestamp'; break;
        case 'set<timestamp>': sql += 'set<timestamp>'; break;
        case 'json': sql += 'text'; break;
        case 'set<json>': sql += 'set<text>'; break;
    ...
    sql += 'primary key (';
    sql += [hashBits.join(',')].concat(rangeBits).join(',') + '))';
    // Execute the table creation query
    return this.client.run_p(sql, [])
    .then(function(){
        tasks = [];
        if (schema.secondaryIndexes) {
            ...
            for (var idx in schema.secondaryIndexes) {
                ...
                tasks.push(self._createIndex(keyspace, indexSchema, 'idx_' + idx +"_ever"));
            }
        }
...
DB.prototype._createIndex = function (keyspace, schema, tableName) {
    ...
    return Promise.all([this.client.run_p(sql)]);
};
