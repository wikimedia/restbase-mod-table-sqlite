...
DB.prototype._get = function (keyspace, req, table, schema) {
    ...
    return self.client.prepare_p(buildResult.query, buildResult.params)
    ...
    .catch(function(err) {
        ...
        if (err instanceof Object && err.cause.code === 'SQLITE_ERROR') {
            return null;
        } else {
            throw err;
        }
...
DB.prototype._put = function(keyspace, req, table ) {
    ...
    if (batch.length === 1) {
        ...
        var query = batch[0];
        mainUpdate = this.client.prepare_p(query.query, query.params);
    } else {
        mainUpdate = this.client.prepare_p(batch);
    }
    return mainUpdate
