...
DB.prototype._createTable = function(keyspace, schema) {
    ...
    var indexViewSql = dbu.buildIndexViewSql(schema, keyspace);
    /*var operation = self.client.run_p('begin transaction')
    .then(function() {
        return P.all([
    ...
    var operation = self.client.run_p(tableSql, [])
    .then(self.client.run_p.bind(self.client, metaSql, []));
    if (staticSql) {
        if (self.conf.show_sql) self.log(staticSql);
        operation = operation.then(self.client.run_p.bind(self.client, staticSql, []));
    }
    if (self.conf.show_sql) self.log(indexViewSql);
    /*operation.then(self.client.run_p.bind(self.client, 'commit transaction'))
    .catch(function(e) {
        console.log('ROLLBACK!!!!', e);
        return self.client.run_p('rollback transaction')
        .finally(function() { throw e; });
    });*/
    return operation.then(self.client.run_p.bind(self.client, indexViewSql, []))
    .catch(function(e) { console.log('ERRRROR', e)});
};

DB.prototype.dropTable = function(domain, table) {
    var self = this;
    var keyspace = dbu.keyspaceName(domain, table);
    var sql = ['drop view ' + keyspace + '_indexView',
        'drop table ' + keyspace + '_meta',
        'drop table ' + keyspace + '_data'];
    if (self.conf.show_sql) sql.forEach(function(s) { self.log(s); });
    return P.each(sql.map(function(query) {
        return self.client.run_p(query, []);
    }));
};
...
DB.prototype._get = function(keyspace, req, table, schema, includePreparedForDelete) {
    ...
    if (self.conf.show_sql) self.log(buildResult.sql);
...
DB.prototype._put = function(keyspace, req, table) {
    ...
    if (self.conf.show_sql) self.log(query.data.sql);
    var operation = self.client.run_p(query.data.sql, query.data.params);
    if (query.static) {
        if (self.conf.show_sql) self.log(query.static.sql);
        operation = operation
        .then(self.client.run_p.bind(self.client, query.static.sql, query.static.params));
    }
    return operation
    .then(function() {
        if (table === 'data') {
    ...
    //.then(self.client.run_p.bind(self.client, 'commit transaction'))
...
DB.prototype._revisionPolicyUpdate = function(keyspace, query, schema) {
    ...
    if (schema.revisionRetentionPolicy.type === 'latest') {
        ...
        revisionPolicyUpdate = self._get(keyspace, dataQuery, 'data', schema, false)
        .then(function(result) {
            if (result.count > schema.revisionRetentionPolicy.count) {
                ...
                return P.all(extraItems.map(function(item) {
                    ...
                    updateQuery.attributes['_exist_until'] = new Date().getTime()
                        + schema.revisionRetentionPolicy.grace_ttl * 1000;
                    var putQuery = dbu.buildPutQuery(updateQuery, keyspace, 'data', schema);
                    if (self.conf.show_sql) self.log(putQuery.data.sql);
                    return self.client.run_p(putQuery.data.sql, putQuery.data.params);
                }))
            }
        });
    } else {
        revisionPolicyUpdate = P.resolve();
    }
    // Step 2: clean up
    return revisionPolicyUpdate.then(function() {
        var deleteExpiredRequest = dbu.buildDeleteExpiredQuery(schema, keyspace);
        if (self.conf.show_sql) self.log(deleteExpiredRequest.sql);
        return self.client.run_p(deleteExpiredRequest.sql, deleteExpiredRequest.params);
    });
};
